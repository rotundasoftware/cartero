...
function Bundle( name, dependencies, files, keepSeparate ) {
...
function Parcel( name, dependencies, files, extendsParcel ) {

	//console.log( "in Parcel constructor" );

	Bundle.apply( this, [ name, dependencies, files, true ] );
...
Parcel.prototype.buildResourcesToLoad = function( staticDir, mode ) {
	var _this = this;
	...
	if( mode === "dev" && options.mode === "prod") {
		_.each( this.combinedFiles, function( file ) {
			filePaths = _.union( filePaths, file.sourceFilePaths );
		} );
	}
	else {
		filePaths = _.pluck( this.combinedFiles, "path" );
	}

	_.each( filePaths, function( filePath ) {
		var fileType = getFileType( filePath );
		_this[ mode ][ fileType ] = _this[ mode ][ fileType ] || [];
		_this[ mode ][ fileType ].push( filePath.replace( staticDir, "" ) );
	} );
};
...
BundlerFactory = {
	...
	createBundle : function( obj ) {
		var bundle = new Bundle( obj.name, obj.dependencies, obj.files, obj.keepSeparate );
		return bundle;
	},

	createParcel : function( obj ) {
		var parcel = new Parcel( obj.name, obj.dependencies, obj.files, obj.extendsParcel );
		return parcel;
	},
