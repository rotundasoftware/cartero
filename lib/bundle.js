	...
	getDependencies : function() {
		...
		_.each( this.dependencies, function( bundleName ) {
			if( bundleName.indexOf( "*" ) !== -1 ) {
				expandedDependencies = _.union( expandedDependencies, assetBundlerUtil.expandDependencyWithWildcard( bundleName, _.keys( this.bundleRegistry ) ) );
		...
		expandedDependencies = _.map( expandedDependencies, function( bundleName ) {
			if( _.isUndefined( _this.bundleRegistry[ bundleName ] ) ) {
				console.log( "Could not find bundle in bundle map: " + bundleName );
			}
			return _this.bundleRegistry[ bundleName ];
		} );
