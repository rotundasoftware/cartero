...
Bundle.prototype = {
	getFilesToServe : function( bundlesAlreadyIncluded, mode, forceKeepSeparate ) {
		...
		if( ! _.isUndefined( this.filesToServe ) ) {
			console.log( "already has filesToServe, returning: " + this.name );
		...
		_.each( dependencies, function( dependency ) {
			if( _.contains( bundlesAlreadyIncluded, this ) )
				throw new Error( "CIRCULAR DEPENDENCY! AHHH! " + this.name );
	...
	mergeFiles : function( files ) {
		...
		_.each( filesByType, function( files, fileType ) {
			...
			if( ! _.isUndefined( _this.filesToServe) && _.contains( _.pluck( _this.filesToServe, "hash" ), filePathsHash ) ) {
				...
				combinedFile.path = path.join( _this.getBundleDirectory(), _this.name.substring( _this.name.lastIndexOf( "/" ) + 1 ) + "_" + filePathsHash ) + "." + fileType;
				...
				if( _.isUndefined( _this.filesToServe ) )
					_this.filesToServe = [];
...
function createHash( filePaths ) {
	return crypto.createHash( "md5" ).update( filePaths.join( "," ) ).digest( "hex" );
}
