...
function createRegistryForDirectory( directory, rootDir, options, dirOptions ) {
	...
	if( _.contains( _.keys( files ), kBundleFileName ) ) {
		...
		if( ! _.isUndefined( bundleJSON[ "browserifyExecuteOnLoad" ] ) )
			browserifyExecuteOnLoad = _.map( bundleJSON[ "browserifyExecuteOnLoad" ], function( autorunFiles ) {
	...
	bundleRegistry[ bundleName ] = new Bundle( {
		...
		browserifyExecuteOnLoad : browserifyExecuteOnLoad,
