...
function createRegistryForDirectory( directory, rootDir, options, dirOptions ) {
	...
	var pageFiles = _.filter( _.keys( files ), function( fileName ) {
		return _.contains( dirOptions.viewFileExt, carteroUtil.getFileExtension( fileName ) );
	} );
		...
		if( fileStats.isDirectory() ) {
			...
			if( dirOptions.directoriesToFlatten.test( fileName ) ) {
				var assetFiles = _.filter( findit.sync( walker.fullPath( fileName ) ), function( fileName ) {
					...
					return carteroUtil.isAssetFile( fileName, options.validOriginalAssetExt );
					} );
				fileDependencies = _.union( fileDependencies, _.map( assetFiles, function( assetFile ) {
					return carteroUtil.mapAssetFileName( path.join( dirOptions.destDir, assetFile.substring( rootDir.length ) ), options.assetExtensionMap );
				} ) );
			}
			else {
			...
			if( carteroUtil.isAssetFile( fileName, options.validOriginalAssetExt ) ) fileDependencies.push( path.join( dirOptions.destDir, walker.fullPath( carteroUtil.mapAssetFileName( fileName, options.assetExtensionMap ) ).substring( rootDir.length + 1) ) );
...
Parcel.createRegistry = function( dirs, bundleRegistry, options ) {
	...
	console.log( "done with first parcel registry population" );
	...
	console.log( "calling getFilesToServe" );
