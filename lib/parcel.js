...
Parcel.prototype.getFilesToServe = function( bundlesAlreadyInParcel, mode, forceKeepSeparate ) {
	...
	if( this.getExtendsParcel() ) {
		...
		files = this.getExtendsParcel().getFilesToServe( bundlesAlreadyInParcel, mode, false );
	}
...
Parcel.prototype.buildResourcesToLoad = function( mode ) {
	var _this = this;
	...
	if( mode === "prod" ) {
		...
		_.each( devFilePaths, function( filePath ) {
	...
	_.each( this.combinedFiles , function( file ) {
		...
		_this[ mode ][ fileType ].push( file.path );
	} );
