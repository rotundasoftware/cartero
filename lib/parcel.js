	...
	_.each( pageFiles, function( fileName ) {
		...
		if( !_.isNull( carteroRequiresMatches ) ) {
			try {
				...
				if( directiveParamsString.lastIndexOf( "'") !== -1 )
					directiveParamsString = directiveParamsString.substring(0, directiveParamsString.lastIndexOf( "'") + 1 );
				else
					directiveParamsString = directiveParamsString.substring(0, directiveParamsString.lastIndexOf( "\"" ) + 1 );
		...
		if( !_.isNull( carteroExtendsMatches ) ) {
			var directiveParamsString = carteroExtendsMatches[1];
			extendsParcel = directiveParamsString;
		}
