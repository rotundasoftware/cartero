...
function buildParcelRegistry( directory, rootDir, options, dirOptions ) {
	...
	var pageFiles = _.filter( _.keys( files ), function( fileName ) {
		return _.contains( dirOptions.viewFileExt, getFileExtension( fileName ) );
	} );
	...
	_.each( _.keys( files ), function( fileName ) {
		...
		if( fileStats.isDirectory() ) {
			...
			if( dirOptions.directoriesToFlatten.test( fileName ) ) {
				...
				_.extend( parcelRegistry, buildParcelRegistry( walker.fullPath( fileName ), rootDir, options, dirOptions ) );
			}
		}
			...
			if( exports.isAssetFile( fileName, options.validOriginalAssetExt ) ) fileDependencies.push( path.join( dirOptions.destDir, walker.fullPath( mapAssetFileName( fileName, options.assetExtensionMap ) ).substring( rootDir.length + 1) ) );
	...
	_.each( pageFiles, function( fileName ) {
		...
		if( !_.isNull( bundlerExtendsMatches ) ) {
			extendsParcel = resolveFullPageName( pageName, bundlerExtendsMatches[1] );
		}

		parcelRegistry[ pageName ] = CarteroFactory.createParcel( {
			name : pageName,
			folder : pageFolder,
			files : _.map( fileDependencies, function( filePath ) {
				return CarteroFactory.createFile( {
					path : filePath,
					keepSeparate : false
				} );
			} ),
...
function buildBundleRegistry( directory, rootDir, options, dirOptions ) {
	...
	_.each( _.keys( files ), function( fileName ) {
		...
		if( fileStats.isDirectory() ) {
			if( _s.startsWith( fileName, "__" ) ) {
				...
				_.extend( bundleRegistry, buildBundleRegistry( walker.fullPath( fileName ), rootDir, options, dirOptions ) );
			}
		}
		else if( fileStats.isFile() ) {
			if( exports.isAssetFile( fileName, options.validOriginalAssetExt ) ) {
	...
	bundleRegistry[ bundleName ] = CarteroFactory.createBundle( {
		name : bundleName,
...
exports.buildBundleRegistry = function( dirs, options ) {
	var bundleRegistry = {};
	_.each( dirs, function( dirOptions ) {
		_.extend (bundleRegistry, buildBundleRegistry( dirOptions.path, dirOptions.path, options, dirOptions ) );
	} );
...
exports.buildParcelRegistry = function( dirs, options ) {
	...
	_.each( dirs, function ( dirOptions ) {
		_.extend( parcelRegistry, buildParcelRegistry( dirOptions.path, dirOptions.path, options, dirOptions ) );
	} );

	_.each( parcelRegistry, function( parcel ) {
		if( ! _.isUndefined( parcel.extendsParcel ) )
