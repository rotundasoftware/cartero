...
function buildPagesMap( directory, rootDir, options ) {
	...
	var pageFiles = _.filter( _.keys( files ), function( fileName ) {
		return options.pageFileRegExp.test( fileName );
	...
	_.each( _.keys( files ), function( fileName ) {
		...
		if( fileStats.isDirectory() ) {
			...
			if( exports.isAssetFile( fileName ) ) {
				...
				var resolvedFileName = walker.fullPath( mapAssetFileName( fileName ) ).substring( rootDir.length + 1 );
...
function resolveAndInjectDependencies( pagePath, bundleMap, pageMap, assetBundlerConfig, rootDir, staticDir, mode ) {
	...
	_.each( pageMetadata.requiredBundles, function( bundleName ) {
		...
		if( ! bundle.keepSeparate && mode === "dev" ) {
			_.each( bundle.files, function( fileName ) {
				...
				filesReferenced.push( fileName.replace( "{ASSET_LIBRARY}/", assetBundlerConfig.assetLibraryDest ) );
			} );
		}

		_.each( bundle.dynamicallyLoadedFiles, function( fileName ) {
			filesReferenced.push( fileName.replace( "{ASSET_LIBRARY}/", assetBundlerConfig.assetLibraryDest ) );
		} );
	...
	_.each( pageMetadata.files, function( file ) {
		...
		filesReferenced.push( file.replace( "{APP_PAGES}/", assetBundlerConfig.appPagesDest ) );
	} );
	...
	_.each( files, function( fileName ) {
		...
		if( suffix === "tmpl" && mode === "prod" ) {
			modeFiles[ "tmpl_cache" ] = modeFiles[ "tmpl_cache" ] || [];
			modeFiles[ "tmpl_cache" ].push( fs.readFileSync( staticDir + path.sep + fileName ) + "\n")
		}
		...
		if( _s.endsWith( fileName, ".js" ) )
			...
			tmplFiles.push( fileName.substring(1) );
		}
...
*/
	...
	//pageMetadata[mode].bundler_js = jsScriptEls;
	//pageMetadata[mode].bundler_css = cssLinkEls;
	//pageMetadata[mode].bundler_tmpl = tmplContents;

	//pageMetadata.tmplFiles = tmplFiles;
...
exports.resolveAndInjectDependencies = function( bundleMap, pageMap, assetBundlerConfig, rootDir, staticDir, mode ) {
	...
	_.each( _.keys( pageMap ), function( pageName ) {
		...
		filesReferenced = _.union( filesReferenced, resolveAndInjectDependencies( pageName, bundleMap, pageMap, assetBundlerConfig, rootDir, staticDir, mode ) );
