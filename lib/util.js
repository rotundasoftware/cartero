...
function buildBundlesMap( directory, rootDir, options ) {
	...
	if( _.contains( _.keys( files ), kBundleFileName ) ) {
		...
		var priFiles = bundleJSON[ "priorityFiles" ];
		...
		if( ! _.isUndefined( priFiles ) ) {
			priorityFiles = _.map( priFiles, function( fileName ) {
	...
	if( priorityFiles.length > 0 ) {
		files = _.union( priorityFiles, _.difference( files, priorityFiles ) );
	}

	//if priority files are specified, push them to the beginning
	if( priorityFiles.length > 0 ) {
		fileDependencies = _.union( priorityFiles, _.difference( fileDependencies, priorityFiles ) );
	}
...
exports.resolveAndInjectDependencies = function( bundleMap, pageMap, assetBundlerConfig, rootDir, staticDir ) {

	_.each( _.keys( pageMap ), function( pageName ) {

		resolveAndInjectDependencies( pageName, bundleMap, pageMap, assetBundlerConfig, rootDir, staticDir );
