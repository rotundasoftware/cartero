...
module.exports = function( packagePathsToIds, outputDirUrl ) {
		...
		function write( buf ) {
			...
			res = res.replace( /##url\(\ *(['"])([^']*)\1\ *\)/, function( wholeMatch, quote, assetSrcPath ) {
				...
				var url = pathMapper( assetSrcAbsPath, function( srcDir ) {
					return packagePathsToIds[ srcDir ] ? '/' + packagePathsToIds[ srcDir ] : null; // return val of outputDirPath needs to be absolute path
				} );
				...
				if( outputDirUrl ) {
					var baseUrl = outputDirUrl[0] === path.sep ? outputDirUrl.slice(1) : outputDirUrl;
					url = baseUrl + url;
				}
